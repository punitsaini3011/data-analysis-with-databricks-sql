...
Ingest Data Using Databricks SQL</h2>
    ...
    USING csv 
    OPTIONS (
        path='wasb://courseware@dbacademy.blob.core.windows.net/data-analysis-with-databricks/v02/web_events/web_events.csv',
        header="true",
        inferSchema="true"
...
The LOCATION Keyword</h2>
    <div class="instructions-div">
    <p>For those of you taking the full course, we talked about the <span class="monofont">LOCATION</span> keyword in the last module. This section is a review, and you can safely skip it. If your taking the short course, or if you need to review the use of <span class="monofont">LOCATION</span>, keep reading.</p>
    <p>We can ingest data in-place using the <span class="monofont">LOCATION</span> keyword to create an external/unmanaged table. There is a dataset, called Sales, that is currently in an object store. Running the command below creates an external table that is associated with this dataset.</p>
    <ol start="6">
        <li>Run the code below.</li>
    </ol>
...
COPY INTO</h2>
    ...
    <p>The second command copies data from an object store location into the <span class="monofont">gym_logs</span> table. Note that the file type for the files in the object store location is specified as "JSON". The last part of the <span class="monofont">COPY INTO</span> command is a file name, a list of file names, or a directory of files.</p>
    </div>
    
    """, statements=["DROP TABLE IF EXISTS gym_logs;", "CREATE TABLE gym_logs (first_timestamp DOUBLE, gym Long, last_timestamp DOUBLE, mac STRING);", """COPY INTO gym_logs 
    FROM 'wasbs://courseware@dbacademy.blob.core.windows.net/data-analysis-with-databricks/v02/gym_logs/gym_logs_json'
    FILEFORMAT = JSON
...
Privileges</h2>
    ...
    """, statements="SHOW GRANT ON SCHEMA {schema_name};") 
...
<h3>GRANT</h3>
    ...
    """, statements=["GRANT USAGE ON SCHEMA {schema_name} TO `{username}`;", 
                     "GRANT SELECT ON SCHEMA {schema_name} TO `users`;"]) 
