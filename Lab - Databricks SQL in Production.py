...
# MAGIC %md 
# MAGIC # Lab: Databricks SQL in Production
...
Refreshing Queries</h2>
    ...
    </div>""", test_code="""DROP TABLE IF EXISTS sales_stream;
...
""", statements=["DROP TABLE IF EXISTS sales_stream;", """CREATE TABLE sales_stream (5minutes STRING, 
                                         clicked_items ARRAY<ARRAY<STRING>>, 
                                         customer_id STRING, 
                                         customer_name STRING, 
                                         datetime STRING, 
...
Alerts</h2>
    ...
    </div>""", test_code="""WITH all_sold_products AS
    (SELECT explode(ordered_products) AS products FROM sales_stream)
SELECT sum(products[3]) AS total_sales FROM FILL_IN;


""", statements="""WITH all_sold_products AS
    (SELECT explode(ordered_products) AS products FROM sales_stream)
SELECT sum(products[3]) AS total_sales FROM all_sold_products;""", label="""What is the total amount of income (in whole dollars) currently in the <span class="monofont">sales_stream</span> table? (type only numbers) """, expected="249003", length=10)
...
""", test_code=None, statements=None, label="""After sharing the dashboard with the "All Users" group, how many rows of users and groups are in the "Manage permissions" dialog? """, expected="3", length=10)
...
""", test_code=None, statements=None, label="""When you click "Advanced" under the "Refresh" drop down, how many minutes are selected by default? (enter only numbers) """, expected="10", length=10)
